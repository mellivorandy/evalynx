...
export default function Index({ auth, teams = [] }) {
    return (
        <div className="flex flex-col min-h-screen bg-gray-50 text-gray-800 dark:bg-black dark:text-white min-h-screen relative">
            <Header auth={auth} />
            <SidePanel auth={auth} />
                                    ...
                                    team.teammembers.length > 0 ? (
                                        team.teammembers.map((member) => (
                                            <li key={member.id}>
                                                學號：{member.user_id}
                                                {member.user
                                                    ? `，姓名：${member.user.name}（${member.user.email}）`
                                                    : "，未知學生"}
                                            </li>
                                        ))
                                    ) : (
